...
- **샘플 프롬프트 필수** — `public/catalog.json` 의 모든 스킬은 `public/sample-prompts/{name}.json` 에 예시 프롬프트 파일을 **반드시** 가진다. 카탈로그에 스킬을 추가하면 같은 커밋에서 이 파일도 만든다.
  ...
  - 스킬명을 그대로 나열하거나 영어 용어를 채워넣기용으로 늘어놓지 말 것. 각 줄은 곧바로 붙여넣어 쓸 수 있는 완결된 지시문.
  - **번역 품질(직역 금지)** — 영어 원문을 기계적으로 직역하지 말 것. 자연스러운 한국어가 우선이되, 한글로 옮기면 어색하거나 뜻이 흐려지는 기술 용어는 **영어를 그대로** 쓴다.
    - 영어 유지가 옳은 예: `git blame`·`diff`·`PR`·`node_modules`·`YAGNI`·파일명(`PONYTAIL-DEBT.md`)·스킬명(`ponytail-debt` 등)·`over-engineering`.
