...
claude-checkup에 Google AdSense 광고를 붙이는 절차. 코드는 이미 연결돼 있고(`NEXT_PUBLIC_ADSENSE_ID` 게이트), 아래는 **사람이 하는 부분**이다.
...
2. Redeploy → 사이트 `<head>`에 AdSense 스크립트가 로드되고 `ads.txt`가 `https://claudecowork.co.kr/ads.txt`로 노출된다(승인 심사가 이 파일을 확인).
3. 광고는 **쿠키 동의 후에만** 로드된다(EU/정책 준수). 동의 배너에서 허용해야 광고가 뜬다.

## 3단계 — AdSense 사이트 추가·심사

1. AdSense → **사이트 메뉴 → 사이트 추가** → `claudecowork.co.kr`
2. AdSense가 주는 심사용 스니펫은 우리 스크립트가 이미 head에 넣으므로 별도 삽입 불필요(확인만).
